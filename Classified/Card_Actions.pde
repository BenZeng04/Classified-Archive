...
public void placeCard(int player, Card baseCard, int x, int y, boolean spawned)
{
  ...
  if(temp.name.equals("Megan"))
    temp.HP = ceil(p[temp.player].HP / 2.0); // HP is player's HP / 2
  int countNovelty = 0;
  for(Card c: playField)
  ...
  if(temp.name.equals("Richard") && mode == 0)
  {
    ...
    {
      int rand = parseInt(random(p[player % 2 + 1].hand.size()));
    ...
    if(mode == 0)
    {
      ...
      for(Card d: collection)
        if(d.category.contains(6))
          tempC.add(d);
      Card c = tempC.get(parseInt(random(tempC.size()))).copy();
      c.summoned = true;
  ...
  if(temp.name.equals("Yebanow") && mode == 0)
  {
    ...
    for(int i = 0; i < 4; i++)
    {
      ...
      int rand = parseInt(random(tempC.size()));
  ...
  if(!spawned && mode == 0) // Cards must NOT be spawned out of any spell/card.
  {
    ...
    for(Card d: playField)
    {
      ...
      {
        Card c = toSummon.get(parseInt(random(toSummon.size()))).copy();
        c.summoned = true;
        c.spawned = true;
        c.player = y;
        add.add(c);
    ...
    while(add.size() > 0)
    {
      ...
      if(availible.size() > 0)
      {
        int i = availible.get(parseInt(random(availible.size())));
...
public void attackPlayer(int attacker)
{
  ...
  int atk = calculateAttack(playField.get(attacker));
...
public void attackCard(int attacker, int takeHit, boolean loop) // Logic for when a CARD GETS ATTACKED BY ANYTHING. This can be the PLAYER, SPELLS, AND CARDS
{
  ...
  int spellAttack = -69420; // Special temporary variable for spells
  if(attacker == -1) playerSelected = true; // Player's attack acts as a 5 damage spell
  if(attacker < -1) { spellAttack = attacker; attacker = -1; loop = false;} // ALL spells
  ...
  // Setting up attack damage
  int atk;
  hit = new Card();
  if(attacker != -1) // If not a player attacking
  {
    hit = playField.get(attacker);
    atk = calculateAttack(hit); // Card Attack
  } 
  else 
    atk = 5; // Player Attack
  ...
  if(spellAttack != -69420) atk = spellAttack * -1;
  ...
  if(attacker > -1) // Setting up attack nerfs/ buffs. Generally this requires the attacker to be a card and not a spell.
  {
    ...
    if(defName.equals("Selina") && hit.cost <= 2) atk = 0; 
    if(defName.equals("Bonnie") && hit.HP > 8) atk = 0;
    if(hit.category.contains(1) && defName.equals("Florence")) atk = 3;
  }
  // General Damage reduction effects that are NOT reliant on there being an attacking card. This means that this effects spell attacks too! 
  boolean has8H = false;
  for(Card c: playField)
  {
  ...
  if(attacker != -1)
  {
    ...
    if(playField.get(takeHit).HP <= 0 && !hasEffect(playField.get(takeHit), "Resurrect"))
    {
      ...
      {
        int tempValue;
        tempValue = parseInt(random(p[opp].hand.size()));
  ...
  if(defName.equals("Disastrous Experiment"))
    playField.get(takeHit).player = playField.get(takeHit).player % 2 + 1;
...
private boolean name(Card c, String name) // Not useful anymore since nullify is gone/reworked
{
...
public void checkDeaths() // Death Effects, and removing cards when dead
{
  ...
  for(Card c: tempRemove)
  {
    ...
    if(c.name.equals("Physouie") && mode == 0)
    {
      ...
      for(int i = 0; i < 3; i++)
      {
        int rand = parseInt(random(temp.size()));
    ...
    if(c.name.equals("UNNAMED2"))
    {
      for(Card d: playField)
        if(max(abs(d.x - c.x), abs(d.y - c.y)) <= 1 && c != d) attackCard(-8, playField.indexOf(d), true);
    }
    if(c.name.equals("UNNAMED3"))
    {
      ArrayList <Integer> availible = new ArrayList<Integer>();
      int yPos = c.y;
      for(int i = 1; i <= 5; i++)
      ...
      for(int i = 0; i < 2; i++)
      {
        if(availible.size() > 0)
        {
          Card d = collection[searchCard("Disastrous Experiment")].copy();
          d.summoned = true;
          d.spawned = true;
          d.player = c.player;
          int var = availible.get(parseInt(random(availible.size())));
          availible.remove(availible.indexOf(var));
          placeCard(c.player, d, var, yPos, true);
        }
      }
    }
  }
  for(int i: specialRemove) // Adding cards that are getting removed via special removal cards
  {
    tempRemove.add(playField.get(i));
    playFieldSelected = -1;
  }
  specialRemove.clear();
  for(Card c: tempRemove) // Finally getting rid of the cards
    playField.remove(c);
}
