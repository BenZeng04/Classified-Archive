...
Add more new world cards
 - When Dead, summon two unobtainable cards in random lanes
 - 2 range, 4 MVMT card which can attack over cards and has 18 attack, 15 HP. This card deals damage to the 8 adjacent cards when dead.
Settings!
Renaming Spells
...
int categCount = 7;
int [] categTot = new int [categCount]; // Total amount of cards per category. This is used for sorting purposes/design!
int [] categColour = new int [categCount];
String [] categNames = new String [categCount];
// Fonts
...
int oldHP1 = 40, oldHP2 = 40; // Old HP of both players at the start of a turn.
...
public void setup()
{
  ...
  //BGM = new SoundFile(this, "BPM.mp3");
  //BGM.play();
  size(1200, 800);
  ...
  p[2] = new Player();
  // Toggleable Buttons
  settings[0] = new Button(900, 320);
  settings[1] = new Button(900, 400);
  
  // Setting Category Stats
  categNames[0] = "Class G"; 
  categColour[0] = 0xff759AE8;
  categNames[1] = "Class H"; 
  categColour[1] = 0xffF2C42C;
  categNames[2] = "Elite"; 
  categColour[2] = 0xffE51515;
  categNames[3] = "Non-Elite"; 
  categColour[3] = 0xff529EFF;
  categNames[4] = "Traveller"; 
  categColour[4] = 0xff16BC0B;
  categNames[5] = "Prototype"; 
  categColour[5] = 200;
  categNames[6] = "Novelty"; 
  categColour[6] = 0xffB536F5;
  //categNames[7] = "New World"; 
  //categColour[7] = #FF34B8;
...
public void setupGame() // Starting Game
{
  ...
  {
    int tempValue = parseInt(random(temp1.size()));
    p[1].deck.add(temp1.get(tempValue));
    temp1.remove(tempValue);
    tempValue = parseInt(random(temp2.size()));
...
public int findCard(int x, int y, int player) // An inefficient way of locating cards for attacking/placing, that greatly minimizes chances of a Concurrent Modification Exception.
{
...
{
  animateTimer++;
  if (animateTimer == 15 && replayOn())
    handOverEffects(playerTurn % 2 + 1);
  else if (animateTimer % 60 == 15 && replayOn()) // Every Second
  {
          ...
          if (m.name.equals("Antnohy") && !hasEffect(playField.get(m.targeted), "Invincible"))
          {
            ...
            for (int l = 0; l < 9; l++)
            {
              ...
              {  
                ...
                if (findCard(tx, ty, m.player) != -1)
                  attackCard(playFieldSelected, findCard(tx, ty, m.player), true);
              }
            }
          }
            ...
            for (int l = 1; l <= 6; l++)
            {
              if (l != m.y) 
              {
                if (findCard(x, l, m.player) != -1)
                  attackCard(playFieldSelected, findCard(x, l, m.player), true);
              }
            }
          }
        }
...
public void drawCard()
{
  ...
  for (Card c : p[playerTurn].hand)
    if (!c.summoned) countNaturalCards++;
  if (p[playerTurn].deck.size() > 0 && (ruleset == 1 || countNaturalCards < 5))
  {
    for (Card c : playField)
      if (c.name.equals("Mr. Filascario") && c.player == playerTurn) p[playerTurn % 2 + 1].HP = max(p[playerTurn % 2 + 1].HP - 1, 0);
...
public void drawCard(int opp)
{
  ...
  for (Card c : p[opp].hand)
    if (!c.summoned) countNaturalCards++;
  if (p[opp].deck.size() > 0 && (ruleset == 1 || countNaturalCards < 5))
  {
    for (Card c : playField)
    {
...
public void addEffect(int duration, int index, String name) // Adding effect to card using index
{
  ...
  Effect e = new Effect();
  e.duration = duration;
  if (mode == 0) e.givenBy = playerTurn; 
  else e.givenBy = playerTurn % 2 + 1;
  e.name = name;
...
{
  return settings[0].on;
}
public boolean replayOn()
{
  return settings[1].on;
}
