public void useSpell(String name, int indexEffect)
{
  ...
  {
    attackCard(-14, indexEffect, true); // -2: Fireball, -3: Holy Hand Grenade Center, -4 
  }
  if(name.equals("Zoo Wee Mama!"))
  {
    attackCard(-9, indexEffect, true); // -2: Fireball, -3: Holy Hand Grenade Center, -4 
  }
  if(name.equals("Mini Gulag"))
  {
    for(int l = 0; l < 9; l++)
    {
      ...
      {
        playField.get(findCard(x, y, Effected.player)).ATK = max(0, playField.get(findCard(x, y, Effected.player)).ATK - 6);
        playField.get(findCard(x, y, Effected.player)).HP = max(0, playField.get(findCard(x, y, Effected.player)).HP - 6);
      }
    }
  }
  if(name.equals("Holy Hand Grenade"))
  {
    attackCard(-11, indexEffect, true);
    for(int l = 0; l < 9; l++)
    {
      if(l != 4)
      {
        ...
        y += l / 3 - 1;
        if(findCard(x, y, Effected.player) != -1)
          attackCard(-8, findCard(x, y, Effected.player), true);
      }
    }
  }
...
public void useSpell(int y, String name) // y is player.
{
  ...
  if(name.equals("Propaganda Machine"))
  {
    if(mode == 0)
    {
      ...
      Card c = temporary.get(parseInt(random(temporary.size()))).copy();
      c.cost --;
      c.summoned = true;
      p[y].hand.add(c);
    }
  ...
  if(name.equals("Elite's Calling") && mode == 0)
  {
    ...
    for(int i = 0; i < 3; i++)
    {
      if(availible.size() > 0)
      {
        Card c = Elites.get(parseInt(random(Elites.size()))).copy();
        c.summoned = true;
        c.spawned = true;
        int j = availible.get(parseInt(random(availible.size())));
