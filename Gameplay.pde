...
{  
  ...
  for(Card c : playField)
  {
    ...
    if(!hasEffect(c, "Nullify"))
    {
      if(c.name == "Joseph")
      {
        for(Card d: playField)
        {
          if(d.name == "Annika" && d.player == c.player)
            atk *= 3;
        }
      }
      ...
      if(c.name == "A.L.I.C.E.")
      {
        for(Card d: playField)
        {
          if(d.name == "Moonlight" && d.player == c.player)
            atk *= 2;
        }
      }
      ...
      if(c.name == "Ms. Nicke")
      {
        for(Card d: playField)
        {
          if(d.name == "Esther" && d.player == c.player)
            atk += 7;
        }
      }
    ...
    if(c.name == "Mr. Pegamah" && !hasEffect(c, "Nullify")) atk = 4;
    if(hasEffect(c, "NVW")) rng++;
    if(hasEffect(c, "JPStun")) mvmt -= 2; 
    
    if(!hasEffect(c, "NoEffect"))
    {
      for(Card d: playField)
      {
        if(d.name == "Ridge Rhea" && c.name != "Ridge Rhea" && c.name != "Ultrabright" && d.player == c.player && !hasEffect(d, "Nullify") && d.x == c.x) rng += 2;
      }
      for(Card d: playField)
      {
        if(d.name == "Ben" && c.category.contains(2) && d.player == c.player && !hasEffect(d, "Nullify")) atk += 3;
      }
      for(Card d: playField)
      {
        if(d.name == "Rita" && d.player != c.player && !hasEffect(d, "Nullify")) atk = max(0, atk - 1);
      }
    }
  ...
  if(cardSelected != -1) // Drawing the selected card OVER everything else
  {
    ...
    if(p[playerTurn].hand.get(cardSelected).isSpell)
    {
      if(p[playerTurn].hand.get(cardSelected).targetsYou) // placed on your own cards
      {
        ...
        for(Card c: playField)
        {
          if(c.player == playerTurn && !hasEffect(c, "NoEffect") && !(name == "Defense Position" && !c.NBTTags.contains("Unhealable")) && !(name == "Dragon Wings" && (c.name == "Ultrabright")))
          {
            hitCircle(c.x, c.y);
          }
        }
  ...
  if(playFieldSelected != -1)  
  {
    ...
    {  
      ...
      if((playField.get(playFieldSelected).NBTTags.contains("SpecialMove")) && (!hasEffect(playField.get(playFieldSelected), "Nullify")))
      {
        if(canDiscard)
        {
          fill(0xffB516F5);
      ...
      color darker = 0;
      color lighter = 0;
      if(pName == "Hubert") darker = 0xff095A81; 
      if(pName == "Ethan") darker = 0xff013107;
      if(pName == "Ms. Iceberg") darker = 0xff810945;
      if(pName == "Hubert") lighter = 0xff03ADFF; 
      if(pName == "Ethan") lighter = 0xff36A744;
      if(pName == "Ms. Iceberg") lighter = 0xffFF0582;
      if(pName == "Hubert") displayText = "Heal"; 
      if(pName == "Ethan") displayText = "Bomb";
      if(pName == "Ms. Iceberg") displayText = "Special Attack";
    ...
    if(choice == 0)
    {
      if(playField.get(playFieldSelected).name == "Hubert" && !hasEffect(playField.get(playFieldSelected), "Nullify") && playField.get(playFieldSelected).player == playerTurn)
      {
        for(Card c: playField)
        {
      ...
      if((playField.get(playFieldSelected).name == "Ethan" || playField.get(playFieldSelected).name == "Ms. Iceberg") && !hasEffect(playField.get(playFieldSelected), "Nullify") && playField.get(playFieldSelected).player == playerTurn)
      {
        for(Card c: playField)
        {
          if(c.player != (playField.get(playFieldSelected).player) && !hasEffect(c, "NoEffect") && (c.cost < 5 || playField.get(playFieldSelected).name != "Ethan"))
          {
            hitCircle(c.x, c.y);
          }
        }
    ...
    {   
      ...
      if(hasEffect(playField.get(playFieldSelected), "JPStun")) mvmt -= 2; 
    ...
    if(choice == 2)
    {
      for(int q = 0; q < 4; q++)
      {
        ...
        for(Card d: playField)
        {
          if(d.name == "Ridge Rhea" && playField.get(playFieldSelected).name != "Ridge Rhea" && playField.get(playFieldSelected).name != "Ultrabright" && d.player == playField.get(playFieldSelected).player && !hasEffect(d, "Nullify") && d.x == playField.get(playFieldSelected).x) rng += 2;
        ...
        for(int i = 1; i <= rng; i++)
        {
          ...
          if(cap)
          {
            ...
            if(playField.get(playFieldSelected).turnPlacedOn != p[playerTurn].turn || (playField.get(playFieldSelected).name == "Mr. Pegamah" && !hasEffect(playField.get(playFieldSelected), "Nullify"))) { firstTurn = false;}
          ...
          for(Card c: playField)
          {
            if(c.x == mx && c.y == my)
            {
              if((playField.get(playFieldSelected).name != "Matthew" && playField.get(playFieldSelected).name != "Ultrabright" && c.player != playerTurn) || hasEffect(playField.get(playFieldSelected), "Nullify"))
                availible = false;
  ...
  {
    if(playField.get(abilitySelected).name == "Jason C" || playField.get(abilitySelected).name == "Esther" || playField.get(abilitySelected).name == "Jefferson" || playField.get(abilitySelected).name == "Jawnie Dirp")
    {
      for(Card c: playField)
      {
    ...
    if(playField.get(abilitySelected).name == "Mandaran" || playField.get(abilitySelected).name == "George" || playField.get(abilitySelected).name == "Anthony")
    {
      for(Card c: playField)
      {
        if(c.player == (playField.get(abilitySelected).player) && !hasEffect(c, "NoEffect"))
...
public void cardDisplay(int x, int y, float scale, Card c, int specialCase)
{
  ...
  imageMode(CENTER);
  textSize(12);
  int textThickness = (int) (textWidth(c.name) / 90) * 20 + 20;
  int co = 0; // Colour
  ...
  if(!mousePressed && cardSelected != -1) // When a card is released
  {
    ...
    if(true) // OOPS
    {
      ...
      for(Card c: playField)
      {
        ...
        if(c.name == "Snake" && c.player == playerTurn && !hasEffect(c, "Nullify"))
        {
          if(temp < 4)
            canPlaceDown = false;
        }
      ...
      if(p[playerTurn].hand.get(cardSelected).isSpell && p[playerTurn].hand.get(cardSelected).cost <= p[playerTurn].cash && canPlaceDown)
      {
        if(p[playerTurn].hand.get(cardSelected).targetsYou) // placed on your own cards
        {
          ...
          for(Card c: playField)
          {
            if(c.player == playerTurn && !hasEffect(c, "NoEffect") && !(p[playerTurn].hand.get(cardSelected).NBTTags.contains("AttackBoostSpell") && c.NBTTags.contains("Unbuffable")) && !(name == "Defense Position" && c.NBTTags.contains("Unhealable")) && !(name == "Dragon Wings" && (c.name == "Ultrabright")))
            {
