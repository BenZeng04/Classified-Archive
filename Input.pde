...
if(mode == 0 && clickDelay == 0 && !inAnimation && !inTransition)
{
  if(abilitySelected == -1)
  {
    ...
    { 
      ...
      if(!hasEffect(playField.get(playFieldSelected), "Nullify")) // No Special Effects. Only attack and move.
      {
        if(choice == 0 && playField.get(playFieldSelected).player == playerTurn)
        {
          ...
          {
            int sideTarget;
            if(playField.get(playFieldSelected).name == "Hubert") sideTarget = playField.get(playFieldSelected).player; else sideTarget = playField.get(playFieldSelected).player % 2 + 1;
            boolean condition = true;
            if(playField.get(playFieldSelected).name == "Hubert") condition = !c.NBTTags.contains("Unhealable");
            if(playField.get(playFieldSelected).name == "Ethan") condition = c.cost < 5;
            ...
            if(availible)
            {
              ...
              if(playerTurn == 1)
              {
                if(dist(cursorX, cursorY, updateX * 100 + 50, updateY * 100 + 50) < 50)
                {
                  ...
                  if(playField.get(playFieldSelected).name != "Simon" || hasEffect(playField.get(playFieldSelected), "Nullify"))
            ...
            if(canAttack) 
            {
              ...
              else
              {
                ...
                if(dist(cursorX, cursorY, mx * 100 + 50, yBound) < 50)
                {
                  ...
                  // Antnohy effect 
                  if(playField.get(playFieldSelected).name == "Antnohy" && !hasEffect(playField.get(playFieldSelected), "Nullify") && !hasEffect(playField.get(takeHit), "Invincible"))
                  {
                  ...
                  if(playField.get(playFieldSelected).name == "GeeTraveller" && !hasEffect(playField.get(playFieldSelected), "Nullify") && !hasEffect(playField.get(takeHit), "Invincible"))
                  {
                    int x = playField.get(takeHit).x, y = playField.get(takeHit).y;
                    for(int l = 1; l <= 6; l++)
                    {
    ...
    else
    {
      ...
      else if(playField.get(abilitySelected).name == "Mandaran" || playField.get(abilitySelected).name == "George" || playField.get(abilitySelected).name == "Anthony")
      {
        for(Card c: playField)
        {
          int yPos = c.y * 100 + 50; if(playerTurn == 2) yPos = c.y * -100 + 750;
  ...
  if((mode == 4 || mode == 5) && clickDelay == 0 && !inAnimation && !inTransition)
  {
    ...
    for(int i = 0; i < collection.length && !addingCard; i++)
    {
      ...
      {
        if(c.name == collection[i].name) { alreadyInDeck = true; break; }
      }
      if(y >= 280 && y <= 730)
      {
